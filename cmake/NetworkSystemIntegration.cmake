...
function(setup_container_system_integration target)
    ...
    # Check for CMake targets created by UnifiedDependencies or add_subdirectory
    if(TARGET ContainerSystem)
        target_link_libraries(${target} PUBLIC ContainerSystem)
        target_compile_definitions(${target} PRIVATE BUILD_WITH_CONTAINER_SYSTEM)
        message(STATUS "Configured ${target} with ContainerSystem CMake target")
    elseif(TARGET container_system)
        target_link_libraries(${target} PUBLIC container_system)
        target_compile_definitions(${target} PRIVATE BUILD_WITH_CONTAINER_SYSTEM)
        message(STATUS "Configured ${target} with container_system CMake target")
    elseif(TARGET container)
        target_link_libraries(${target} PUBLIC container)
        target_compile_definitions(${target} PRIVATE BUILD_WITH_CONTAINER_SYSTEM)
        message(STATUS "Configured ${target} with container CMake target")
    elseif(CONTAINER_SYSTEM_TARGET)
    ...
    elseif(CONTAINER_SYSTEM_INCLUDE_DIR)
        ...
        target_compile_definitions(${target} PRIVATE BUILD_WITH_CONTAINER_SYSTEM)
        message(STATUS "Configured ${target} with container_system paths (include path)")
    else()
        message(STATUS "${target}: container_system not found, container integration disabled")
    endif()
endfunction()
...
function(setup_thread_system_integration target)
    ...
    # First check if ThreadSystem CMake target exists (from UnifiedDependencies, FetchContent or add_subdirectory)
    ...
    elseif(TARGET thread_system)
        # Alternative target name from UnifiedDependencies
        target_link_libraries(${target} PUBLIC thread_system)
        target_compile_definitions(${target} PRIVATE BUILD_WITH_THREAD_SYSTEM)
        set(_thread_integration TRUE)
        message(STATUS "Configured ${target} with thread_system CMake target")
    elseif(TARGET thread_base AND TARGET interfaces AND TARGET utilities)
        # Link individual thread_system components (from add_subdirectory)
        target_link_libraries(${target} PUBLIC thread_base interfaces utilities)
        target_compile_definitions(${target} PRIVATE BUILD_WITH_THREAD_SYSTEM)
        set(_thread_integration TRUE)
        message(STATUS "Configured ${target} with thread_system components (thread_base, interfaces, utilities)")
    ...
    elseif(THREAD_SYSTEM_INCLUDE_DIR)
        ...
        target_compile_definitions(${target} PRIVATE BUILD_WITH_THREAD_SYSTEM)
        set(_thread_integration TRUE)
        message(STATUS "Configured ${target} with thread_system integration (include path)")
    endif()
    ...
    if(_thread_integration)
        ...
        message(WARNING "BUILD_WITH_THREAD_SYSTEM is ON but no thread_system target found!")
    endif()
endfunction()
...
function(setup_logger_system_integration target)
    ...
    # First check if LoggerSystem CMake target exists (from UnifiedDependencies, FetchContent or add_subdirectory)
    ...
    elseif(TARGET logger_system)
        target_link_libraries(${target} PUBLIC logger_system)
        target_compile_definitions(${target} PRIVATE BUILD_WITH_LOGGER_SYSTEM)
        message(STATUS "Configured ${target} with logger_system CMake target")
    elseif(TARGET logger)
        target_link_libraries(${target} PUBLIC logger)
        target_compile_definitions(${target} PRIVATE BUILD_WITH_LOGGER_SYSTEM)
        message(STATUS "Configured ${target} with logger CMake target")
    ...
    elseif(LOGGER_SYSTEM_INCLUDE_DIR)
        ...
        target_compile_definitions(${target} PRIVATE BUILD_WITH_LOGGER_SYSTEM)
        message(STATUS "Configured ${target} with logger_system integration (include path)")
    else()
...
function(setup_monitoring_system_integration target)
    ...
    if(MONITORING_SYSTEM_TARGET)
        ...
        target_compile_definitions(${target} PUBLIC BUILD_WITH_MONITORING_SYSTEM)
...
function(setup_common_system_integration target)
    ...
    if(TARGET kcenon::common_system)
        target_link_libraries(${target} PUBLIC kcenon::common_system)
        target_compile_definitions(${target} PUBLIC BUILD_WITH_COMMON_SYSTEM)
        message(STATUS "Configured ${target} with kcenon::common_system CMake target")
    elseif(TARGET common_system)
        target_link_libraries(${target} PUBLIC common_system)
        target_compile_definitions(${target} PUBLIC BUILD_WITH_COMMON_SYSTEM)
        message(STATUS "Configured ${target} with common_system CMake target")
    elseif(TARGET common)
        target_link_libraries(${target} PUBLIC common)
        target_compile_definitions(${target} PUBLIC BUILD_WITH_COMMON_SYSTEM)
        message(STATUS "Configured ${target} with common CMake target")
    elseif(COMMON_SYSTEM_TARGET)
    ...
    elseif(COMMON_SYSTEM_INCLUDE_DIR)
        ...
        target_compile_definitions(${target} PUBLIC BUILD_WITH_COMMON_SYSTEM)
        message(STATUS "Configured ${target} with common_system integration (include path)")
    else()
        message(WARNING "BUILD_WITH_COMMON_SYSTEM is ON but no common_system target found!")
    endif()
endfunction()
