...
{
protected:
	void SetUp() override { server_ = std::make_shared<messaging_server>("boundary_test_server"); }
...
TEST_F(HttpErrorBoundaryTest, HandlesLongDetail)
{
	...
	auto error = http_error_response::make_error(http_error_code::bad_request, long_detail);
...
{
	std::string special_detail = "Error with \"quotes\" and \\ backslashes and\nnewlines";
	auto error = http_error_response::make_error(http_error_code::bad_request, special_detail);
...
{
	std::string unicode_detail = "Error: \xC3\xA9\xC3\xA0\xC3\xBC"; // UTF-8 characters
	auto error = http_error_response::make_error(http_error_code::bad_request, unicode_detail);
...
{
	auto error = http_error_response::make_error(http_error_code::bad_request, "Test",
