...
{
protected:
	void SetUp() override { server_ = std::make_shared<messaging_server>("failure_test_server"); }
...
TEST_F(NetworkFailureTest, HandlesMultipleDisconnects)
{
	...
	for (int i = 0; i < 10; ++i)
	{
		auto client = std::make_shared<messaging_client>("test_client_" + std::to_string(i));
		auto connect_result = client->start_client("localhost", TEST_PORT);
		ASSERT_TRUE(connect_result.is_ok());
