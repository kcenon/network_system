            ...
            for (int j = 0; j < operations_per_thread; ++j) {
                try {
                    auto client = std::make_shared<messaging_client>(
                        "test_client_" + std::to_string(thread_id) + "_" + std::to_string(j)
                    );
                    ++created;
            ...
            for (int j = 0; j < operations_per_thread; ++j) {
                try {
                    auto server = std::make_shared<messaging_server>(
                        "test_server_" + std::to_string(thread_id) + "_" + std::to_string(j)
                    );
                    ++created;
...
TEST_F(NetworkThreadSafetyTest, ConcurrentServerStartStop) {
    ...
    for (int i = 0; i < num_servers; ++i) {
        servers.push_back(std::make_shared<messaging_server>("server_" + std::to_string(i)));
    }
...
TEST_F(NetworkThreadSafetyTest, ConcurrentClientStartStop) {
    ...
    for (int i = 0; i < num_clients; ++i) {
        clients.push_back(std::make_shared<messaging_client>("client_" + std::to_string(i)));
    }

    // Start clients from different threads (will fail to connect, but that's OK for this test)
    for (int i = 0; i < num_clients; ++i) {
        threads.emplace_back([&, index = i]() {
            try {
...
TEST_F(NetworkThreadSafetyTest, MixedOperations) {
    ...
    threads.emplace_back([&]() {
        for (int i = 0; i < num_iterations; ++i) {
            try {
                auto server = std::make_shared<messaging_server>("mixed_server_" + std::to_string(i));
    ...
    threads.emplace_back([&]() {
        for (int i = 0; i < num_iterations; ++i) {
            try {
                auto client = std::make_shared<messaging_client>("mixed_client_" + std::to_string(i));
                std::this_thread::sleep_for(10ms);
            } catch (...) {
                ++errors;
            }
        ...
        threads.emplace_back([&, thread_id = i]() {
            for (int j = 0; j < operations; ++j) {
                try {
                    if (j % 2 == 0) {
                        auto server = std::make_shared<messaging_server>(
                            "stress_server_" + std::to_string(thread_id) + "_" + std::to_string(j)
                        );
                    } else {
                        auto client = std::make_shared<messaging_client>(
                            "stress_client_" + std::to_string(thread_id) + "_" + std::to_string(j)
                        );
                    }
                    ++total_operations;
                } catch (...) {
...
TEST_F(NetworkThreadSafetyTest, MultipleServerPorts) {
    ...
    for (int i = 0; i < num_ports; ++i) {
        servers.push_back(std::make_shared<messaging_server>("multi_port_server_" + std::to_string(i)));
    }
