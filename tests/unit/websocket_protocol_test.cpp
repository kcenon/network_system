...
TEST(WebSocketProtocolTest, ReceiveTextMessage) {
  ...
  auto frame = websocket_frame::encode_frame(
      ws_opcode::text, std::vector<uint8_t>{'H', 'e', 'l', 'l', 'o'}, true,
      false);
...
TEST(WebSocketProtocolTest, ReceiveBinaryMessage) {
  ...
  auto frame = websocket_frame::encode_frame(
      ws_opcode::binary, std::vector<uint8_t>{0x01, 0x02, 0x03}, true, false);
...
TEST(WebSocketProtocolTest, ReceiveFragmentedTextMessage) {
  ...
  auto frame1 = websocket_frame::encode_frame(
      ws_opcode::text, std::vector<uint8_t>{'H', 'e', 'l'},
      false, // Not final
      false);

  // Create continuation fragment (FIN=true)
  auto frame2 = websocket_frame::encode_frame(ws_opcode::continuation,
...
TEST(WebSocketProtocolTest, ReceiveFragmentedBinaryMessage) {
  ...
  auto frame2 = websocket_frame::encode_frame(
      ws_opcode::continuation, std::vector<uint8_t>{0x03, 0x04}, true, false);
...
TEST(WebSocketProtocolTest, ReceiveMultipleFragments) {
  ...
  auto frame2 = websocket_frame::encode_frame(
      ws_opcode::continuation, std::vector<uint8_t>{'B'}, false, false);

  auto frame3 = websocket_frame::encode_frame(
      ws_opcode::continuation, std::vector<uint8_t>{'C'}, true, false);
...
TEST(WebSocketProtocolTest, ReceivePing) {
  ...
  auto frame = websocket_frame::encode_frame(
      ws_opcode::ping, std::vector<uint8_t>{'p', 'i', 'n', 'g'}, true, false);
...
TEST(WebSocketProtocolTest, ReceivePong) {
  ...
  auto frame = websocket_frame::encode_frame(
      ws_opcode::pong, std::vector<uint8_t>{'p', 'o', 'n', 'g'}, true, false);
...
TEST(WebSocketProtocolTest, ReceiveMultipleMessages) {
  ...
  protocol.set_message_callback(
      [&]([[maybe_unused]] const ws_message &msg) { callback_count++; });
...
TEST(WebSocketProtocolTest, ReceivePartialFrame) {
  ...
  bool callback_invoked = false;
  protocol.set_message_callback(
      [&]([[maybe_unused]] const ws_message &msg) { callback_invoked = true; });
