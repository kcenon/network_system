...
**현재 상태:**
- `send_packet()` 호출에 이동 의미론 사용
- 파이프라인이 변환 중 여전히 복사 수행
- scatter-gather I/O 지원 없음
...
**설명:**
파이프라인에서 실제 압축을 구현합니다 (현재는 플레이스홀더 플래그만 존재).

**현재 상태:**
- `compress_mode_` 플래그가 존재하지만 아무 것도 하지 않음
- 파이프라인에 압축/압축 해제 스텁 존재
- 실제 압축 구현 없음
...
| 버전    | 기능                                    | 소요 시간   |
|---------|----------------------------------------|-----------|
| v1.5.0  | 재연결 로직, 상태 모니터링              | 5-7일      |
| v1.6.0  | Zero-Copy 파이프라인, 압축              | 9-12일     |
| v1.7.0  | UDP 신뢰성 계층                         | 8-10일     |
| v2.0.0  | HTTP/2, gRPC, 메트릭 대시보드           | 27-38일    |
| v2.1.0+ | QUIC 프로토콜                           | 15-20일    |

### 구현 로드맵

**Phase 1 (v1.5.0):** 안정성 및 신뢰성
- 재연결 로직
- 상태 모니터링

**Phase 2 (v1.6.0):** 성능 최적화
- Zero-copy 파이프라인
- 메시지 압축

**Phase 3 (v1.7.0):** 프로토콜 향상
- UDP 신뢰성 계층
- DTLS 지원
