...
- **CRTP에서 Composition으로 리팩토링 Phase 1.1-1.2** (2026-01-12) (#411, #422, #423)
  - 종합적인 CRTP 분석 문서 추가 (`docs/design/crtp-analysis.md`)
    ...
    - 보일러플레이트 패턴에서 ~60% 코드 중복 식별
    - 의존성 및 마이그레이션 고려사항 분석
  - Composition 기반 설계 명세서 추가 (`docs/design/composition-design.md`)
    - 새로운 인터페이스 계층 설계 (INetworkComponent, IClient, IServer, ISession)
    - 공유 기능을 위한 유틸리티 클래스 정의
    - 하위 호환성 계획이 포함된 마이그레이션 전략 개요
  - 새로운 인터페이스 클래스 추가 (`include/kcenon/network/interfaces/`)
    - `i_network_component.h`: 모든 네트워크 컴포넌트의 기본 인터페이스
    - `i_client.h`: 라이프사이클 및 콜백이 포함된 클라이언트 측 인터페이스
    - `i_server.h`: 세션 관리가 포함된 서버 측 인터페이스
    - `i_session.h`: 서버-클라이언트 연결을 위한 세션 인터페이스
  - 코드 중복 감소를 위한 유틸리티 클래스 추가 (`include/kcenon/network/utils/`)
    - `lifecycle_manager.h`: 스레드 안전 시작/중지 상태 관리
    - `callback_manager.h`: 범용 스레드 안전 콜백 저장 및 호출
    - `connection_state.h`: 원자적 연산이 포함된 연결 상태 머신
  - 이 컴포넌트들은 CRTP에서 Composition 패턴으로의 마이그레이션 기반 형성
