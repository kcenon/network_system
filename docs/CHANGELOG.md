...
### Added
- **Protocol support-status matrix — stub/placeholder audit ([#1144](https://github.com/kcenon/network_system/issues/1144), part of [#964](https://github.com/kcenon/network_system/issues/964))**
  - Adds `docs/PROTOCOL_SUPPORT.md` classifying every protocol and protocol-adjacent surface as `production` / `experimental` / `test-only` / `remove`, with per-row evidence, disposition, and test references
  - Production protocols (TCP, UDP, TLS/SSL, WebSocket, HTTP/1.1, HTTP/2, HPACK, QUIC, gRPC) confirmed covered by happy-path and non-happy-path tests; existing test inventory referenced per protocol (no new coverage marathon — overall coverage is owned by [#953](https://github.com/kcenon/network_system/issues/953))
  - `experimental` surfaces documented and excluded from the v1.0 stability surface: HPACK Huffman pass-through stub (`src/protocols/http2/hpack.cpp`, already covered by Huffman stub-contract tests), experimental QUIC client `stats()` and `alpn_protocol()` accessors (`src/experimental/quic_client.cpp` — internal header annotated as experimental), DTLS server payload handling (`secure_messaging_udp_server::process_session_data`), QUIC `process_ack_frame` minimal handling (`src/internal/quic_socket.cpp`), and non-OTLP-HTTP tracing exporters (`src/tracing/exporters.cpp`)
  - `test-only` surfaces documented: `libs/network-{http2,grpc}/src/*.cpp` re-export wrappers (real implementation lives in `src/protocols/...`) and standalone-build fallback adapter types
  - All `TODO`/`FIXME`/`HACK`/`stub`/`placeholder`/`not implemented` markers across `include/`, `src/`, `libs/` eliminated or accounted for; documented false positives include gRPC `stub_` member names, HTTP 501 reason phrases, the gRPC `unimplemented` status enum, the `null_connection_observer` doc comment, the `:param` route-syntax comment, and the QUIC packet-number "done separately" comment
  - README protocol feature matrix updated in English (`README.md`) and Korean (`README.kr.md`); benign `http_server.cpp` compression-fallback `TODO` comment clarified; no rows classified `remove`
- **API freeze for v1.0 — public API surface audit ([#1125](https://github.com/kcenon/network_system/issues/1125), part of [#964](https://github.com/kcenon/network_system/issues/964))**
  - Adds `docs/v1.0-api-surface.md` enumerating every public header under `include/kcenon/network/` (excluding the `detail/` subtree) and classifying each top-level symbol as `stable` / `rename-candidate` / `remove-candidate` / `experimental`
  - Public surface scope: 31 headers across 12 functional groups — Umbrella (`network_system.h`, `forward.h`, `utils.h`), Concepts (`concepts/concepts.h`), Configuration (`config/config.h`, `config/feature_flags.h`), Types (`types/result.h`), Policy (`policy/tls_policy.h`), Interfaces (8 files), Facades (5 files: `tcp`/`udp`/`http`/`websocket`/`quic`), Protocol (`protocol/protocol.h`, `protocols/grpc/grpc.h`), Unified (`unified/unified.h`), Session (`session/session.h`), Integration (3 files: `network_system_bridge.h`, `thread_integration.h`, `di/service_registration.h`), Observability (`metrics/metrics.h`, `tracing/tracing.h`, `events/network_metric_event.h`)
  - Audit findings: zero `[[deprecated]]` symbols on the public surface (the only `deprecated` matches in `include/` are inside `detail/utils/result_types.h:44` and `:54` which are not part of the v1.0 contract); zero experimental headers (already migrated to `src/internal/experimental/` per [#577](https://github.com/kcenon/network_system/issues/577) — `include/kcenon/network/experimental/` contains only a `README.md`); zero rename candidates (the camelCase / snake_case mix in `integration/network_system_bridge.h` is preserved as a known stylistic artifact of [#579](https://github.com/kcenon/network_system/issues/579) because rename churn would break documented downstream API)
  - Downstream consumer verification: Tier 5 consumer `pacs_system` was scanned for `kcenon/network` includes — uses only `facade/tcp_facade.h` (in `src/integration/network_adapter.cpp` and `src/network/v2/dicom_server_v2.cpp`), `interfaces/i_session.h`, and `session/session.h`; does not reach into any `detail/` header and does not depend on any deprecated, experimental, or rename-candidate symbol; the facade-pattern direction taken in #577 has held
  - Freeze contract recorded in `docs/v1.0-api-surface.md` "Maintenance After Freeze" section and `CONTRIBUTING.md` "v1.0 API Freeze Policy" section: after v1.0 tag, public headers may not have symbols removed, renamed, or signature-changed without a major version bump; adding new symbols and overloads remains non-breaking; CI gate `.github/workflows/public-api-check.yml` continues to enforce "no `throw` in public headers" against the contract; `detail/` subtree (34 headers) remains free to evolve in v1.x patches without SemVer impact
  - Runtime deprecation note: `policy::no_tls` carries a runtime warning recommending TLS-enabled policies in production (per [#871](https://github.com/kcenon/network_system/issues/871)) but the type itself remains supported and is not marked `[[deprecated]]` because plain-text transport remains a valid development and intra-trust-boundary use case
  - No code changes to public headers in this PR — the audit is documentation-only and the freeze list ships exactly the headers that already exist on `develop` as of 2026-05-10
- **Round 3 known-stream dispatcher coverage for `src/protocols/http2/http2_client.cpp` (#1119)**
  - Adds `tests/unit/http2_client_dispatcher_branch_test.cpp` (38 TEST cases) and extends `tests/support/http2_client_test_access.h` with stream-seeding helpers (`seed_stream`, `set_stream_callbacks`, `stream_state_of`, `stream_window_size_of`, `append_response_header`, `response_body_of`, `build_headers`, `allocate_stream_id`, `next_stream_id`) so direct-dispatch tests can drive the known-stream branches that Round 6 (#1115) and Round 2 (#1117) left untouched
  - New coverage: `handle_rst_stream_frame` known-stream close-and-fulfill branch with both CANCEL and REFUSED_STREAM error codes; `handle_goaway_frame` populated-streams loop body covering streams above `last_stream_id` (closed and promise.set_value) versus at-or-below (kept open) and the already-closed-skip arm; `handle_window_update_frame` known-stream non-zero stream_id branch (per-stream window increment) plus boundary value `INT32_MAX`; `handle_data_frame` non-streaming buffer-append branch, end_stream status-code-extraction success path with status 200, malformed-status `std::stoi` catch arm, streaming `on_data` callback fan-out, streaming end_stream `on_complete` callback path, per-stream WINDOW_UPDATE branch with 33000-byte payload triggering the `< DEFAULT_WINDOW_SIZE/2` guard, connection-level WINDOW_UPDATE branch under the same guard, max-uint31 stream id boundary, and 16384-byte (max_frame_size) payload buffer round-trip; `handle_headers_frame` known-stream end_headers without end_stream (accumulate), end_stream success path decoding `:status: 200` and `:status: 404` from HPACK static-table indexes 8 and 13, malformed-status catch arm via pre-staged response_headers, streaming `on_headers` callback path, streaming end_stream `on_complete` path with status 500 (HPACK static index 14); `process_frame` dynamic-cast-fails / break / fall-through arm for all seven dispatched frame types (settings, headers, data, rst_stream, goaway, window_update, ping) plus the default arm for `priority` / `push_promise` / `continuation` types
  - Private helper coverage via friend forwards: `build_headers` empty-path → "/" substitution branch, pseudo-header sequencing (`:method` / `:scheme` / `:authority` / `:path`) with user-agent prepend, additional-header pseudo-header skip branch (leading-colon names rejected); `allocate_stream_id` RFC 7540 §5.1.1 odd-id contract verification with monotonic +2 increment
  - Production header (`src/internal/protocols/http2/http2_client.h`) compiles byte-identical when `BUILD_TESTS=OFF` because all friend declarations remain inside the existing `#if defined(NETWORK_ENABLE_TEST_INJECTION)` block; no new friend declarations were added — the existing `friend class kcenon::network::tests::support::http2_client_test_access` covers all the new accessor methods because they are member functions of the same class
  - Local verification: `cmake --build build --target network_http2_client_dispatcher_branch_test` succeeds, `./build/bin/network_http2_client_dispatcher_branch_test` reports 38/38 PASSED in 1 ms wall-time on Apple Clang Debug build; existing `network_http2_client_branch_test` non-network suites (34 tests across 8 suites) continue to pass after the access-header extension
  - Acceptance gate (#1119): `src/protocols/http2/http2_client.cpp` line coverage >=70% / branch coverage >=50% — to be confirmed by the post-merge `Coverage Analysis` workflow against the PR #1117 baseline
  - Part of the [#953](https://github.com/kcenon/network_system/issues/953) coverage expansion effort; chains from #1115 Round 6 → #1117 Round 2 → this PR Round 3
- **Round 6 dispatcher coverage pivot for `src/internal/protocols/http2/http2_client.cpp` (#1115, #1116)**
  - Pivots 7 dispatcher-only TEST_F in `tests/unit/http2_client_branch_test.cpp` to friend-injected `process_frame` via a new `tests/support/http2_client_test_access.h` (single dedicated friend class gated by the existing `NETWORK_ENABLE_TEST_INJECTION` macro, matching the `quic_server.h:55` / `websocket_server.h:33` pattern), bypassing the SETTINGS-handshake `wait_for` path that PR #1114 demonstrated to be structurally bounded by the ctest 300 s timeout under coverage instrumentation
  - Load-bearing evidence: test #5868 measured 15.175 s and test #5869 hit the 300 s timeout in Coverage Analysis run [25562347620](https://github.com/kcenon/network_system/actions/runs/25562347620); after this refactor the same seven `Server*` TEST_F complete in 0 ms wall-time under both Debug and Coverage builds — proof that dispatcher branches are now reachable under instrumentation
  - Scope: 7 `Server*` dispatcher TEST_F refactored (`ServerPingFrameDrivesHandlePingAndKeepsConnectionAlive`, `ServerPingAckFrameIsAbsorbedSilently`, `ServerGoawayFrameFlipsConnectionStateToDisconnected`, `ServerWindowUpdateOnConnectionStreamExpandsWindow`, `ServerWindowUpdateOnUnknownStreamIsSilentlyIgnored`, `ServerRstStreamOnUnknownStreamIsSilentlyIgnored`, `ServerUnknownFrameTypeIsHandledWithoutCrashing` — null-pointer guard at `http2_client.cpp:622`); the 14 connect-state public-API TEST_F (`StartStream*`, `WriteStream*`, `CancelStream*`, `SecondConnect*`, `SendRequestTimesOut*`, `PostWithBody*`, `SetSettings*`) genuinely require the connected-state path and remain on the PR #1114 multiplier scaffold — out of scope for Round 6
  - Production header (`src/internal/protocols/http2/http2_client.h`) compiles byte-identical when `BUILD_TESTS=OFF` because the `NETWORK_ENABLE_TEST_INJECTION` macro is undefined in production builds; friend declaration is gated by `#if defined(NETWORK_ENABLE_TEST_INJECTION)` block
  - Post-merge `Coverage Analysis` workflow on develop will produce the definitive lcov measurement against the PR #1109 baseline (LH/LF=108/576 = 18.75% line, BRH/BRF=97/979 = 9.91% branch); acceptance gate per #1115 is ≥+20pp line / ≥+10pp branch on `src/internal/protocols/http2/http2_client.cpp`. If the gate is not met, follow-up decision per #1115 Risks bullet 3 — lower the gate or accept the gap with written justification
  - Part of the [#953](https://github.com/kcenon/network_system/issues/953) coverage expansion effort; chains from PR #1111 Round-4 diagnosis (Option A/B/C analysis) → PR #1113 / PR #1114 Round-5 multiplier (exhausted) → this PR Round-6 friend-injected pivot
- **In-process loopback coverage tests for `src/http/websocket_server.cpp` (#1067)**
  - Added `tests/unit/websocket_server_loopback_test.cpp` registered via `add_network_test`. Drives `messaging_ws_server` end-to-end against a real RFC 6455 client built from `internal::websocket_socket` + `internal::tcp_socket` on a probed free port, reaching branches that the hermetic `tests/unit/websocket_server_branch_test.cpp` (Issue #1053) and `tests/test_messaging_ws_server.cpp` cannot cover without a live handshake
  - Targets >=80% line / >=70% branch coverage on `src/http/websocket_server.cpp` (up from 39.9% line / 19.7% branch as measured in coverage run 24947193873 on develop @ 2026-04-26); CI must confirm the uplift
  - Test groups: lifecycle (start/stop, already-running rejection, `start_server(ws_server_config&)` overload, `i_websocket_server::start(port)` interface delegation, restart cycle, `bind_failed` catch arm with port held by separate acceptor, destructor-while-running); messages (connection_callback firing on connect, text_callback / binary_callback firing on `on_message` dispatch, raw message slot exercise); close (peer-initiated close frame triggering `on_close` and disconnection_callback, connection_count return-to-zero); broadcast (`broadcast_text` and `broadcast_binary` reaching at least one populated session via session_mgr non-null branch); lookup (`get_connection` non-null and `get_all_connections` non-empty after registered handshake, `get_connection` for unknown id after start); `ws_connection` accessors (`id`, `is_connected`, `path`, `remote_endpoint`) and send/close methods (`send_text`, `send_binary`, `send`, `close()`, `close(uint16_t,string_view)`) reachable only after a real handshake; auto-pong (true branch responds, false branch leaves server healthy on client PING); max_connections=0 limit branch in `handle_new_connection`; raw garbage handshake exercising `ws_socket->async_accept` failure branch in `handle_new_connection`; post-stop broadcast / `wait_for_stop` safety; two concurrent clients exercising the do_accept loop more than once
  - File-internal helpers: `probe_free_port` (asio acceptor on port 0 to discover an OS-assigned port and release before the server binds — short race window accepted as a known limitation), `wait_until` (5ms-poll predicate with 2 s default budget), and `WsLoopbackClient` (drives a real handshake, exposes `send_text` / `send_binary` / `send_ping` / `close` / `set_message_handler` / `set_pong_handler`)
  - Placed under `tests/unit/` rather than appended to `tests/test_messaging_ws_server.cpp` because the latter explicitly documents a no-peer / no-io_context invariant; mixing loopback tests there would break that invariant. Aligns with existing convention (`websocket_server_branch_test.cpp`, `websocket_server_test.cpp` for the same class)
  - Part of the #953 coverage expansion effort; complements the hermetic-only #1053 branch tests by exercising `do_start_impl` / `do_stop_impl` success paths, `do_accept` loop, `handle_new_connection` success / max_connections-zero / handshake-failure branches, `on_message` text+binary dispatch, `on_close`, `broadcast_text` / `broadcast_binary` populated-session branches, `get_connection` / `get_all_connections` success branches, the `auto_pong` true and false branches, `~messaging_ws_server` while running, the `bind_failed` catch arm of `do_start_impl`, and `ws_connection` / `ws_connection_impl` methods reachable only after a real connection
